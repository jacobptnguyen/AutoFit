...
- **Production guardrails.** Uploads are capped at 4MB, 50,000 rows, and 500 columns, zero data persistence (everything is processed in memory and discarded once the request ends), and per-IP rate limiting protecting the LLM spend on a public-facing link.
